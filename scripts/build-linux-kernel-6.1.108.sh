...
patch -p1 < ../patches/linux-$KERNEL_VERSION-armv7-fpga.diff
git add --update
...
patch -p1 < ../patches/linux-$KERNEL_VERSION-armv7-fpga-patch-usb-chipidea.diff
git add --update
git commit -m "patch for usb chipidea driver for issue #3"

### Patch for build debian package script
patch -p1 < ../patches/linux-$KERNEL_VERSION-armv7-fpga-patch-builddeb.diff
git add --update
